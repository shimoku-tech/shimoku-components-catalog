...
def button_click_to_new_tab(
        title: str, url: str, background_color: str = 'var(--color-black)',
        hover_background_color: str = 'var(--color-grey-700)', text_color: str = 'var(--background-paper)',
        hover_text_color: str = 'var(--color-blue)', font_size: int = 14, image_url: str = None
) -> str:
    uuid_str = str(uuid1())
    return (
            "<head>"
            ...
            f".button-{uuid_str}"
            f"{{display: flex; width: 100%; height: auto; background-color: {background_color};"  # Change bg color
            "padding: 5% 5%; border-radius: var(--border-radius-xl);"  # Change padding to increase width and height
            f"color: {text_color}; box-shadow: var(--box-shadow-m); transition-duration: 0.3s;}}"
            f".button-{uuid_str}:hover{{background-color: {hover_background_color}; color: {hover_text_color};}}"
            # Change bg and text hover colors
            "</style>"
            # End styles button
            "</head>"
            f"<div class='s-{uuid_str}'>"
            f"<a href='{url}' target='_blank'>"
            f"<div class='button-{uuid_str}'>" +
            (f"<img src='{image_url}' alt='Github Logo' "
             "style='width: 32px; height: 32px; margin: auto; color: var(--color-white)'>"  # logo button
             if image_url else f"<p style='margin: auto; font-size: {font_size}px'>" + title + "</p>") +
            "</div>"
            "</a>"
            "</div>"
    )
